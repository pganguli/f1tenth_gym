...
def main():
    ...
    while not done:
        ...
        if not (keys[pyglet.window.key.W] or keys[pyglet.window.key.A] or keys[pyglet.window.key.D]):
            action = planner.plan(obs, ego_idx=0)
            ego_steer = action.steer
            ego_speed = action.speed

        # Clip actions to valid ranges
        ego_steer = np.clip(ego_steer, env.unwrapped.params["s_min"], env.unwrapped.params["s_max"])
        ego_speed = np.clip(ego_speed, env.unwrapped.params["v_min"], env.unwrapped.params["v_max"])
