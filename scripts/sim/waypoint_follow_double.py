...
def main():
    ...
    if waypoints_orig.size > 0:
        ...
        )  # Transparent White for Original
        env.unwrapped.add_render_callback(render_waypoints_orig)

    # Start both cars at the same position (inter-agent collisions are ignored)
    ...
    while not done:
        # Action for agent 0 (Noisy)
        action_noisy = planner_noisy.plan(obs, ego_idx=0)

        # Action for agent 1 (Original)
        ...
        # Done if either car finishes or crashes (depending on env behavior)
