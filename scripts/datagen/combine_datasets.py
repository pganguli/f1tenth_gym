...
def combine_datasets(
    input_files: List[str], output_file: str, epsilon: float = 1e-4
) -> None:
    """
    Combines multiple .npz datasets and removes redundant rows.

    This function loads multiple NPZ files, checks for key consistency across them,
    concatenates the data, and then performs a deduplication step based on an
...
if __name__ == "__main__":
    ...
    combine_datasets(args.inputs, args.output, args.epsilon)
