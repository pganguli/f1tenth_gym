...
class LongestReachablePathPlanner(BasePlanner):
    @staticmethod
    def getCoordinates(r: float, angle: float) -> np.ndarray:
        return np.array([r * np.cos(angle), r * np.sin(angle)])

    @staticmethod
    def index2Angle(i: int, maxIndex: int, startAngle: float, endAngle: float) -> float:
    ...
    def plan(self, obs: dict[str, Any], ego_idx: int) -> Action:
        ...
        iDirection = (
            1
            if scan_data[originalI + self.adjustment_step]
            > scan_data[originalI - self.adjustment_step]
            else -1
        )
        ...
        while True:
            ...
            maxI = np.clip(maxI, 0, len(scan_data) - 1)
            if maxI == 0 or maxI == len(scan_data) - 1:
