...
from ..utils import get_side_distances, get_heading_error, get_actuation
...
class DynamicWaypointPlanner(BasePlanner):
    ...
    def plan(self, obs: dict[str, Any], ego_idx: int) -> Action:
        ...
        lookahead_point = np.array([
            car_position[0] + target_x_vehicle * np.cos(car_theta) - target_y_vehicle * np.sin(car_theta),
            car_position[1] + target_x_vehicle * np.sin(car_theta) + target_y_vehicle * np.cos(car_theta),
            self.max_speed
        ])
