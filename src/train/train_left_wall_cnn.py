...
y = data['left_wall_dist']  # shape = (num_samples,)
...
X_train_tensor = torch.tensor(X_train, dtype=torch.float32).unsqueeze(1)  # (batch,1,1080)
...
# 5. Save the model
# -----------------------------
torch.save(model.state_dict(), "../data/lidar_leftwall_cnn.pth")
...
model.eval()
with torch.no_grad():
    sample = X_test_tensor[0].unsqueeze(0)  # shape (1,1,1080)
