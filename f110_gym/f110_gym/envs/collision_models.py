...
@njit(cache=True)
def perpendicular(pt: np.ndarray) -> np.ndarray:
    """
...
def idx_furthest_pt(vertices: np.ndarray, d: np.ndarray) -> int:
    ...
    """
    return int(np.argmax(vertices.dot(d)))


@njit(cache=True)
def support(vertices1: np.ndarray, vertices2: np.ndarray, d: np.ndarray) -> np.ndarray:
    """
...
@njit(cache=True)
def collision(vertices1: np.ndarray, vertices2: np.ndarray) -> bool:
    """
...
@njit(cache=True)
def get_vertices(pose: np.ndarray, length: float, width: float) -> np.ndarray:
    """
